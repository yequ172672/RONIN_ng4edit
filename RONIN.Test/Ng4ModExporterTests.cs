...
internal static class Ng4ModExporterTests
{
    ...
    {
        using var temp = new TemporaryDirectory();
        string csv = Path.Combine(temp.Path, "@patch_image1.csv");
        string dat = Path.Combine(temp.Path, "@patch_image1.dat");
        ...
        Assert(Directory.GetFiles(temp.Path, "@image*", SearchOption.TopDirectoryOnly).Length == 0,
            "Exporter created a game archive file.");
    }
