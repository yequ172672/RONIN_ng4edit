...
if (args.Contains("--ng4mod-workspace-tests", StringComparer.OrdinalIgnoreCase))
{
    Ng4ModWorkspaceTests.Run();
    return;
}

if (args.Length == 3 && args[0].Equals("--inspect-ng4-materials", StringComparison.OrdinalIgnoreCase))
{
    Ng4MaterialDebugCli.Run(args[1], args[2]);
    return;
}

if (args.Contains("--ng4-preview-material-tests", StringComparer.OrdinalIgnoreCase))
{
    Ng4PreviewMaterialTests.Run();
    return;
}

if (args.Contains("--ng4-browser-tree-tests", StringComparer.OrdinalIgnoreCase))
{
    Ng4BrowserTreeTests.Run();
    return;
}

if (args.Contains("--host-texture-archive-tests", StringComparer.OrdinalIgnoreCase))
{
    HostTextureArchiveTests.Run();
    return;
}

if (args.Contains("--host-ng4mod-tests", StringComparer.OrdinalIgnoreCase))
{
    HostNg4ModTests.Run();
    return;
}

if (args.Contains("--blender-bridge-tests", StringComparer.OrdinalIgnoreCase))
{
    RngpPreviewTests.Run();
    return;
}

if (args.Length == 2 && args[0].Equals("--generate-ng4mod-fixture", StringComparison.OrdinalIgnoreCase))
{
    Console.WriteLine(Ng4ModExporterTests.GenerateGoldenFixture(args[1]));
    return;
}

if (args.Contains("--run-ng4mod-tests", StringComparer.OrdinalIgnoreCase))
{
    Ng4ModPackageTests.Run();
    Ng4ModCatalogTests.Run();
    ParentPayloadBuilderTests.Run();
    Ng4ModExporterTests.Run();
    return;
}
...
if (args.Contains("--mdl-gltf-tests", StringComparer.OrdinalIgnoreCase))
{
    MdlGltfTests.Run();
    return;
}
...
if (args.Length == 4 && args[0].Equals("--export-model-texture-set-tga", StringComparison.OrdinalIgnoreCase))
{
    TextureSetCli.ExportResult result = TextureSetCli.ExportTga(args[1], args[2], args[3]);
    Console.WriteLine($"Exported {result.TextureCount} root-mip TGA textures for {result.ModelPath} to {result.Destination}");
    return;
}

if (args.Length == 5 && args[0].Equals("--export-ng4mod-workspace", StringComparison.OrdinalIgnoreCase))
{
    string assetsDirectory = Path.GetFullPath(args[1]);
    AssetLibrary workspaceLibrary = AssetLibrary.Load(assetsDirectory);
    AssetEntry workspaceModel = workspaceLibrary.All.Single(entry => entry.Type == AssetType.SkeletalMesh && entry.Path.Equals(args[2], StringComparison.OrdinalIgnoreCase));
    Ng4ModWorkspaceExportResult workspace = Ng4ModWorkspaceService.Export(workspaceModel, workspaceLibrary.All, args[3]);
    string databasePath = Path.Combine(assetsDirectory, "AssetDatabase.dat");
    await using FileStream database = File.OpenRead(databasePath);
    string hash = Convert.ToHexString(await System.Security.Cryptography.SHA256.HashDataAsync(database)).ToLowerInvariant();
    Ng4ModWorkspacePackageResult package = Ng4ModWorkspaceService.Package(workspace.GlbPath, workspaceLibrary,
        new Ng4ModWorkspacePackageRequest("ronin.cli." + workspaceModel.FileName.ToLowerInvariant(), workspaceModel.FileName, "1.0.0", "", "", [], args[4], hash));
    Console.WriteLine($"Workspace: {workspace.WorkspacePath}");
    Console.WriteLine($"Package: {package.PackagePath} ({package.ChangedTextureCount} changed textures)");
    return;
}
...
if (args.Length == 3 && args[0].Equals("--export-mdl-glb", StringComparison.OrdinalIgnoreCase))
{
    MDLFullData exportModel = MDLParserExtended.Parse(File.ReadAllBytes(args[1]));
    MdlToGltfConverter.WriteGlb(exportModel, args[2]);
    Console.WriteLine($"Exported {args[2]}");
    return;
}

if (args.Length == 3 && args[0].Equals("--create-face-triangle-test", StringComparison.OrdinalIgnoreCase))
{
    FaceModelCli.CreateTriangleTest(args[1], args[2]);
    return;
}

if (args.Length == 4 && args[0].Equals("--import-glb-mdl", StringComparison.OrdinalIgnoreCase))
{
    byte[] replacement = GltfToMdlConverter.Convert(args[1], File.ReadAllBytes(args[2]));
    File.WriteAllBytes(args[3], replacement);
    MDLFullData verified = MDLParserExtended.Parse(replacement);
    Console.WriteLine($"Imported {args[1]} -> {args[3]} ({verified.Batches.Length} batches, {verified.Groups.Length} groups)");
    return;
}
