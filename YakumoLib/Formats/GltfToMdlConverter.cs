...
public static partial class GltfToMdlConverter
{
    ...
    private static byte[] BuildTemplateReplacement(ModelRoot gltf, MDLFullData template)
    {
        ...
        if (uvLayerCount > 3 || colorLayerCount > 1 || skinLayerCount > 2)
            throw new NotSupportedException($"Template requires {uvLayerCount} UV, {colorLayerCount} color, and {skinLayerCount} skin layers; the validated GLB layout supports at most 3 game UVs plus one identity UV, 1 color, and 2 skin layers.");
        ...
        out VertexGroup[] groups, out MDLBatch[] batches, out ushort[] indices)
    {
        ...
        List<VertexGroupPlan> groupPlans = BuildVertexGroupPlans(imported, template.Batches, template.Groups.Length);
        groups = new VertexGroup[groupPlans.Count];
        ...
        for (int groupIndex = 0; groupIndex < groups.Length; groupIndex++)
        {
            ...
            foreach (int batchIndex in groupPlans[groupIndex].BatchIndices)
            {
                ...
                if (vertexBase + source.Positions.Length > ushort.MaxValue)
                    throw new InvalidOperationException($"Internal vertex-group packing error: rebuilt group {groupIndex} exceeds 65535 vertices.");
                batches[batchIndex].vertexGroupID = checked((uint)groupIndex);
            ...
            VertexGroup layout = template.Groups[groupPlans[groupIndex].LayoutGroupIndex];
    ...
    private static List<VertexGroupPlan> BuildVertexGroupPlans(
        ImportedBatch?[] imported, MDLBatch[] templateBatches, int templateGroupCount)
    {
        var binsByTemplateGroup = new List<List<List<int>>>(templateGroupCount);
        for (int groupIndex = 0; groupIndex < templateGroupCount; groupIndex++)
        {
            int[] batchIndices = Enumerable.Range(0, templateBatches.Length)
                .Where(batchIndex => templateBatches[batchIndex].vertexGroupID == groupIndex)
                .ToArray();
            if (batchIndices.Length == 0)
                throw new NotSupportedException(
                    $"Template vertex group {groupIndex} is not referenced by any batch and cannot be rebuilt safely.");
            foreach (int batchIndex in batchIndices)
            {
                int vertexCount = imported[batchIndex]!.Positions.Length;
                if (vertexCount > ushort.MaxValue)
                    throw new NotSupportedException(
                        $"MDL batch {batchIndex} contains {vertexCount} vertices; a single batch cannot exceed 65535 vertices and cannot be split automatically.");
            }

            int totalVertexCount = batchIndices.Sum(batchIndex => imported[batchIndex]!.Positions.Length);
            binsByTemplateGroup.Add(totalVertexCount <= ushort.MaxValue
                ? new List<List<int>> { batchIndices.ToList() }
                : PackBatchesIntoVertexGroups(batchIndices, imported));
        }

        // Preserve every template group at its original index. Overflow bins are appended in
        // source-group order so an unchanged template always retains identical group IDs.
        var plans = new List<VertexGroupPlan>(templateGroupCount + binsByTemplateGroup.Sum(bins => Math.Max(0, bins.Count - 1)));
        for (int groupIndex = 0; groupIndex < templateGroupCount; groupIndex++)
            plans.Add(new VertexGroupPlan(groupIndex, binsByTemplateGroup[groupIndex][0].Order().ToArray()));
        for (int groupIndex = 0; groupIndex < templateGroupCount; groupIndex++)
            foreach (List<int> overflowBin in binsByTemplateGroup[groupIndex].Skip(1))
                plans.Add(new VertexGroupPlan(groupIndex, overflowBin.Order().ToArray()));
        return plans;
    }

    private static List<List<int>> PackBatchesIntoVertexGroups(int[] batchIndices, ImportedBatch?[] imported)
    {
        // Deterministic best-fit decreasing: place large batches first, then choose the bin that
        // leaves the least capacity. It keeps batches intact and normally reaches the minimum
        // group count without introducing an expensive/explosive exact bin-packing search.
        var bins = new List<List<int>>();
        var binVertexCounts = new List<int>();
        foreach (int batchIndex in batchIndices
                     .OrderByDescending(index => imported[index]!.Positions.Length)
                     .ThenBy(index => index))
        {
            int vertexCount = imported[batchIndex]!.Positions.Length;
            int bestBin = -1;
            ...
            {
                int remaining = ushort.MaxValue - binVertexCounts[binIndex] - vertexCount;
            ...
            if (bestBin < 0)
            {
                bins.Add(new List<int> { batchIndex });
                binVertexCounts.Add(vertexCount);
            }
            else
            {
                bins[bestBin].Add(batchIndex);
                binVertexCounts[bestBin] += vertexCount;
            }
        }
        return bins;
    }

    private sealed record VertexGroupPlan(int LayoutGroupIndex, int[] BatchIndices);
