...
public static class MDLParserExtended
{
    ...
    public static string[] ReadMaterialNames(byte[] data)
    {
        ...
        if (BinaryPrimitives.ReadUInt32LittleEndian(bytes) != MagicNumber)
            throw new InvalidDataException("Not a valid MDL file.");

        uint declaredSize = BinaryPrimitives.ReadUInt32LittleEndian(bytes[4..]);
        if (declaredSize > data.Length)
            throw new InvalidDataException("MDL file size exceeds the available data.");
        int length = declaredSize == 0 ? data.Length : checked((int)declaredSize);
        if (length < HeaderFieldCount * 4)
            throw new InvalidDataException("MDL declared file size is smaller than its header.");

        uint count = BinaryPrimitives.ReadUInt32LittleEndian(bytes[0x34..]);
        uint tableOffset = BinaryPrimitives.ReadUInt32LittleEndian(bytes[0x38..]);
        if (count > int.MaxValue || (ulong)tableOffset + (ulong)count * 4 > (ulong)length)
            throw new InvalidDataException("MDL material names range is outside the file.");

        var names = new string[(int)count];
        for (int index = 0; index < names.Length; index++)
        {
            int pointerOffset = checked((int)tableOffset + index * 4);
            uint stringOffset = BinaryPrimitives.ReadUInt32LittleEndian(bytes[pointerOffset..]);
            if (stringOffset >= length)
                throw new InvalidDataException("MDL material names range is outside the file.");
            int start = checked((int)stringOffset);
            int terminator = bytes[start..length].IndexOf((byte)0);
            if (terminator < 0)
                throw new InvalidDataException("Unterminated material names string.");
            try { names[index] = StrictUtf8.GetString(bytes.Slice(start, terminator)); }
            catch (DecoderFallbackException exception)
            { throw new InvalidDataException("MDL material name is not valid UTF-8.", exception); }
        }
        return names;
    }
    ...
    public static MDLFullData Parse(byte[] data)
    {
        ...
        string lodName = h.LODNameOffset == 0 ? string.Empty : ReadStringPointer(reader, data.Length, h.LODNameOffset, "LOD name");
        ...
        return new MDLFullData
        {
            ...
            LODName = lodName, OriginalBytes = (byte[])data.Clone()
        };
    }
    ...
    private static string ReadStringPointer(BinaryReader r, int length, uint tableOffset, string label)
    {
        RequireRange(length, tableOffset, 4, label);
        r.BaseStream.Position = tableOffset;
        return ReadCString(r, length, r.ReadUInt32(), label);
    }
