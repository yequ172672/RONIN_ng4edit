...
public static class MDLWriter
{
    ...
    public static byte[] Write(MDLFullData data)
    {
        ...
        ValidateTemplateContract(template, data);
    ...
    private static byte[] BuildSections(MDLFullData data, bool preserveTemplateBytes)
    {
        ...
        WriteStringPointer(writer, data.LODName, out header.LODNameOffset);
    ...
    private static void ValidateTemplateContract(MDLFullData template, MDLFullData edited)
    {
    ...
    private static void WriteStringPointer(BinaryWriter w, string value, out uint tableOffset) { Align(w, 4); tableOffset = Position(w); w.Write(0u); uint offset = Position(w); WriteCString(w, value, out _); long end = w.BaseStream.Position; w.BaseStream.Position = tableOffset; w.Write(offset); w.BaseStream.Position = end; }
    ...
    private static void Reject(string structure) => throw new NotSupportedException($"Template-safe MDL replacement does not support changing {structure}; use MDLWriter.Rebuild for a verified topology rebuild.");
}
