...
public static class Ng4MaterialInstanceReader
{
    ...
    {
        if (data.Length < 0x54 || BinaryPrimitives.ReadUInt32LittleEndian(data.Slice(8, 4)) != 7)
            throw new InvalidDataException("Instance.dat header is invalid.");
        for (int index = 0; index < 7; index++)
            if (BinaryPrimitives.ReadUInt32LittleEndian(data.Slice(12 + index * 8, 4)) != index)
                throw new InvalidDataException("Instance.dat section identifiers are invalid.");
        int parentOffset = checked((int)BinaryPrimitives.ReadUInt32LittleEndian(data.Slice(16, 4)) + 4);
        if (parentOffset != 0x44)
            throw new InvalidDataException($"Instance.dat parent UUID begins at unexpected offset 0x{parentOffset:X}.");
        UUID candidate = ReadUuid(data, parentOffset);
        UUID? parent = candidate.a != 0 || candidate.b != 0 || candidate.c != 0 || candidate.d != 0 ? candidate : null;

        var textures = new Dictionary<string, UUID>(StringComparer.OrdinalIgnoreCase);
        // The fourth of the seven NG4 sections (table entry at 0x24/0x28) stores texture parameters.
        int textureSection = checked((int)BinaryPrimitives.ReadUInt32LittleEndian(data.Slice(0x28, 4)) + 4);
        foreach (NamedUuidRecord record in NamedUuidRecord.ReadContainer(data, textureSection, "Instance.dat texture section"))
            textures[record.Name] = record.AssetId;
        return new Ng4MaterialInstanceData(parent, textures);
    }
