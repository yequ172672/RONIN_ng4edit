...
public static class TexturePackageDds
{
    ...
    private static void ValidateDdsHeader(byte[] ddsBytes, uint expectedTotalSize, uint? expectedMipCount, out int width, out int height, out int mipCount, bool validateExactSize)
    {
        int ddsHeaderLength = GetDdsHeaderLength(ddsBytes);
        ...
        if ((flags & (DdsPitchFlag | DdsLinearSizeFlag)) == 0)
