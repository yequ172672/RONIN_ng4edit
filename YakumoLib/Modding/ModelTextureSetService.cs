...
public sealed record ModelTextureSetManifest(int SchemaVersion, string ModelAssetId, string ModelPath, DateTimeOffset ExportedAtUtc, IReadOnlyList<ModelTextureSetEntry> Textures);
...
public static class ModelTextureSetService
{
    ...
    public static IReadOnlyList<ModelTextureCandidate> DiscoverCandidates(AssetEntry model, IEnumerable<AssetEntry> assets)
    {
        ...
        return assets.Where(a => a.Type == AssetType.Texture)
    ...
    public static string ClassifyUsage(string fileName)
    {
        ...
        if (tokens.Any(t => t.Equals("BaseCol", StringComparison.OrdinalIgnoreCase))) return "BaseCol";
    ...
    public static ModelTextureSetManifest Export(AssetEntry model, IEnumerable<AssetEntry> assets, string destinationDirectory)
    {
        ...
        ModelTextureCandidate[] candidates = DiscoverCandidates(model, assets).ToArray();
        ...
        foreach (var item in planned)
        {
            ...
            try { extracted = TexturePackageDds.ExtractRootMip(item.Candidate.Asset, contentDirectory); }
            catch (Exception ex) { throw new InvalidDataException($"Failed to extract texture '{item.Candidate.Asset.Path}'.", ex); }
        ...
        try
        {
            ...
            var manifest = new ModelTextureSetManifest(1, model.StringAssetID, model.Path, DateTimeOffset.UtcNow, prepared.Select(item => item.ManifestEntry).ToArray());
        ...
        Action<Image<Rgba32>, string> saveImage)
    {
        ...
        var candidates = DiscoverCandidates(model, assets).ToArray();
        ...
        try
        {
            ...
            var manifest = new ModelTextureSetManifest(schemaVersion, model.StringAssetID, model.Path, DateTimeOffset.UtcNow, entries);
    ...
    public static ModelTextureImportPlan PlanImport(string directory, IEnumerable<AssetEntry> assets, bool compress = false)
    {
        ...
        if (manifest.SchemaVersion is not (1 or 2) || manifest.Textures is null)
        ...
        foreach (ModelTextureSetEntry item in manifest.Textures)
        {
            ...
            {
                if (!item.DdsFile.EndsWith(".png", StringComparison.OrdinalIgnoreCase))
                    throw new InvalidDataException($"PNG texture set entry '{item.DdsFile}' has the wrong file extension.");
                using Image<Rgba32> image = Image.Load<Rgba32>(ddsPath);
                if (image.Width != item.Width || image.Height != item.Height) throw new InvalidDataException($"PNG '{item.DdsFile}' dimensions differ from its manifest.");
    ...
    private static void ValidateManifestEntry(ModelTextureSetEntry entry)
    {
        ...
        if (string.IsNullOrWhiteSpace(entry.DdsFile) || entry.DdsFile != Path.GetFileName(entry.DdsFile) ||
            ...
            !(entry.DdsFile.EndsWith(".dds", StringComparison.OrdinalIgnoreCase) || entry.DdsFile.EndsWith(".png", StringComparison.OrdinalIgnoreCase)))
            throw new InvalidDataException($"Manifest texture filename '{entry.DdsFile}' is unsafe.");
        if (entry.Width <= 0 || entry.Height <= 0 || entry.MipCount <= 0 || entry.DxgiFormat is not (71 or 72 or 77 or 78 or 80 or 83))
